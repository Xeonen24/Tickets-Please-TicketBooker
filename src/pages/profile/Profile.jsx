    ...
    axios
      ...
      .then((response) => {
        const formattedBookings = response.data.map((booking) => {
          const selectedSeats = booking.selectedSeats.map((seat) => {
            return `${seat.row} - ${seat.column} - ${seat.tier}`;
          });
				...
				const config = 
          {
            ...
            headers: {
              "Content-Type": "application/json",
				...
				const res = await axios.post('http://localhost:5000/api/update-password', body, config);
